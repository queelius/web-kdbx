...
### add_entry/
- Origin: copied from diff-kdbx (generated by `cargo run --features cli --bin gen-fixtures`).
- Master password: test-password-do-not-use
- Contents: `before` is empty; `after` contains one entry under Root with Title=Test Entry, UserName=alice, Password=hunter2 (protected), URL=https://example.com.
- Regen: re-copy from `~/github/kdbx/diff-kdbx/tests/fixtures/add_entry/`. If the layout above changes, update this manifest, `tests/e2e/browse-and-reveal.spec.ts`, and any other spec asserting on entry text in the same commit.
